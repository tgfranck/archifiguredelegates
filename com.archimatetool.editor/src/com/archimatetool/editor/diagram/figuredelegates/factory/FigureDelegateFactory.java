...
public class FigureDelegateFactory {
	...
	/**
	 * Register the Figure Delegates defined in the Figure Delegates extension
	 * point.
	 */
	...
	private void registerDelegates() {
		...
		for (IConfigurationElement configurationElement : registry
			...
			try {
				...
				Class<? extends IFigure> delegateFor = (Class<? extends IFigure>) Class
						.forName(configurationElement.getAttribute("delegateFor"));

				IFigureDelegateProvider provider = FigureDelegateProviderFactory.INSTANCE
						.getProviderForClass(delegateFor);
				
				if (id != null && delegateClass != null && delegateFor != null && provider != null) {
	...
	/**
	 * @param delegateClass
	 *            the type of delegate that should be created
	 * @param owner
	 *            the figure that should own the delegate
